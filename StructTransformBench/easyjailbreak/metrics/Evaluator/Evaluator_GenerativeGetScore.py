...
Your rating should strictly follow this format: "Rating: [[rating]]", the rating in the double-closed brackets is a number from 1 to 10, e,g, "Rating: [[5]]"."""
    ...
    def _evaluate(self, instance:Instance, score_format=None, judge_prompt=None, system_prompt=None, **kwargs):
        ...
        score = self.get_score(seed=judge_prompt, score_format=score_format, **kwargs)
        logging.info(f"Score is as follows: {score}")
    ...
    def get_score(self, seed:str, score_format, **kwargs):
        ...
        # error handling for lack of response
        try:
            raw_output = self.eval_model.generate(seed)
        except Exception as e:
            print("Judge failed to response:", e)
            return 10

        logging.info(f"Judge model's raw ouptut: {raw_output}")
